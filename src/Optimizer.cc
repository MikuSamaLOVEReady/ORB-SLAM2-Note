...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2, const bool bFixScale)
{
    ...
    void Optimizer::SparseOptimizationLocalBA(KeyFrame *pFrame, Map* pMap ,
        ...
        std::cout << " allPonts: " << allMapPoints.size() << std::endl;
        ...
        for( auto it = ans.begin() ; it!=ans.end() ;++it){
                ...
                if( index <= graph.GetGraphsize()-2 ){
                    ...
                    if( index >=0  && allKeyFrames[index] != nullptr )
                        lLocalKeyFrames.insert(allKeyFrames[index]);
                }
        }
        ...
        for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
        {
            ...
            for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
            {
                ...
                if(pKFi->mnBALocalForKF!=pFrame->mnId && pKFi->mnBAFixedForKF!=pFrame->mnId)
                {
                    pKFi->mnBAFixedForKF=pFrame->mnId;
                    if(!pKFi->isBad())
                        lFixedCameras.push_back(pKFi);
                }
            }
...
//                bDoMore = false;
    ...
    pair<int, int> Graph::minCostMaxFlow(int source, int sink, int flowLimit) {
            ...
            int flow = 0;
            int cost = 0;
            int sumSize = mapPointCount + covisKeyFrameCount + 2;
